...
SecureCmdList["EQUIP_TO_SLOT"] = function(msg)
	local slot, rest = strmatch(msg, "%s*(%d+)%s*(.*)");
	EquipItemByName(rest, slot);
end

SecureCmdList["USE"] = function(msg)
	if ( msg ~= "" ) then
		local bag, slot = strmatch(msg, "^%s*(%d+)%s+(%d+)%s*$");
		if ( not bag ) then
			slot = strmatch(msg, "^%s*(%d+)%s*$");
		end
		if ( bag ) then
...
SecureCmdList["CHANGEACTIONBAR"] = function(msg)
	ChangeActionBarPage(msg);
end

SecureCmdList["SWAPACTIONBAR"] = function(msg)
	local a, b = strmatch(msg, "%s*(%d+)%s+(%d+)");
